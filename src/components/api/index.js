...
export default {
    ...
    subscribeBars: (symbolInfo, interval, onRealtimeCallback, subscribeUID, onResetCacheNeededCallback) => {
        ...
        let lastCandle = {
            time: +historicCandles[historicCandles.length-1].time,
            open: +historicCandles[historicCandles.length - 1].open,
            high: +historicCandles[historicCandles.length - 1].open + 0.0001*8,
            low: +historicCandles[historicCandles.length - 1].open,
            close: +historicCandles[historicCandles.length - 1].open,
            volume: 1.12893,
        }
